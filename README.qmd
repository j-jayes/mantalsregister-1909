...
We can go back to correct 1830 and 1840. If you are in a hurry to do this, the code called [scraper](code/03-scraper.qmd) is the one you can use. It's easy to follow.
...
```{r}
df %>% 
  filter(year %in% c(1830, 1840, 1880)) %>% 
  count(year, page) %>% 
  ggplot(aes(x = page, y = n, fill = factor(year))) +
  geom_col() +
