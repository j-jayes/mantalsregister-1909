...
df %>% 
  write_rds(here::here("data", "export", "mantalsregister_data.rds"))

df %>% 
  write_excel_csv(here::here("data", "export", "mantalsregister_data.csv"))

df %>% 
  write_dta(here::here("data", "export", "mantalsregister_data.dta"))

tibble(
  type = c("Stata format", "CSV", "RDS"),
  links = c(
    "data/export/mantalsregister_data.dta",
...
records %>% 
  ...
  tab_header(title = "Number of records per year", 
             subtitle = "On web portal")

records %>% 
  ggplot(aes(year, n_records)) +
  geom_line() +
